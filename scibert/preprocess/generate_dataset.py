...
def main():
    ...
    if not os.path.isfile(args.input_train_data) or not os.path.isfile(args.input_dev_data):
        raise ValueError(
            "The input data file path ({} or {}) does not exist.".format(args.input_train_data, args.input_dev_data))

    nb_train_sent = 0
    nb_test_sent = 0
    fo = open(os.path.join(args.output_dir, "train.txt"), "w")
    with open(args.input_train_data, 'r') as fi:
        for line in fi:
    ...
    fo = open(os.path.join(args.output_dir, "dev.txt"), "w")
    with open(args.input_dev_data, 'r') as fi:
        for line in fi:
            # count the number of sentences
            if len(line.strip()) == 0:
                nb_test_sent += 1
    ...
    print("Number of evaluation sentences: {}".format(nb_test_sent))
